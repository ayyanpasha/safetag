...
describe('mobile payments API', () => {
  it('getSubscription calls correct endpoint', async () => {
    (api.get as any).mockResolvedValue({ success: true, data: { plan: 'YEARLY' } });
  ...
  it('createSubscription sends plan and vehicleId', async () => {
    ...
    await createSubscription('MONTHLY', 'v1');
    expect(api.post).toHaveBeenCalledWith('/api/payments/subscribe', { plan: 'MONTHLY', vehicleId: 'v1' });
  });
