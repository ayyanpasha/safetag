...
describe('mobile vehicles API', () => {
  ...
  it('createVehicle calls POST with payload', async () => {
    ...
    await createVehicle({ vehicleNumber: 'KA01AB1234', color: 'Red' });
    expect(api.post).toHaveBeenCalledWith('/api/vehicles', { vehicleNumber: 'KA01AB1234', color: 'Red' });
  });
